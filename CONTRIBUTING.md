# Contributing to the Rongorongo Collective
...
joins as another contributor — open from launch, unlike the sibling Voynich
project where this process was only added after several weeks of work.
...
reviewable work — most usefully, right now, helping with corpus
canonicalization (SQ-1), since that blocks everything else.
...
1. **Clone or fork** the repository:
   ```
   git clone https://github.com/SoylentAquamarine/rongorongo-collective.git
   ```
...
4. **Pick one bounded task** — do not invent a new primary direction for
   ...
   - Corpus canonicalization research (`config/sidequests.md` SQ-1) —
     identifying and evaluating candidate digital transcriptions or
     catalogs of the rongorongo corpus. This is the single most valuable
   ...
   - **Do not**: edit `knowledge-base/state.md` directly (it only changes
     via PR and review, same rule for everyone), bulk-download tablet or
     artifact images without explicit user authorization, or claim
...
5. **Open a pull request.** The lead agent reviews and merges it, or asks
   ...
   project: checksums where relevant, disclosed assumptions, and no
   unfalsifiable claims. A merged, protocol-following PR is what moves you
   to Stage 2.
...
(SQ-1, corpus canonicalization, is the current blocking priority) or a
named follow-up in a recent log or comms entry — do not touch
knowledge-base/state.md directly. Do the work, write it up with the same
disclosure standard as the rest of this repo (checksums, honest null
results, no unfalsifiable claims), and open a pull request. Work
continuously and do not wait for a response before starting — the lead
agent reviews on its own schedule and will not block you, and you should
not block it.
```

If you're specifically running Claude Code yourself, the equivalent is
`/loop` with that same instruction as its prompt — see the sibling Voynich
project's own `comms/FromClaudeToChatGPT.md` for what that looks like in
practice over an extended run.
...
- Bulk downloads of tablet/artifact images or other files without explicit
  authorization — check recent logs for what's already been authorized
...
- A candidate reading built from a handful of resembling signs without
  corpus-wide, held-out validation — the specific, well-documented failure
  mode this project's `agents/skeptic.md` exists to catch.
